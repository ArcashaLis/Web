...
for (let i = 0; i < tableContentMatrix.length; i++) {
    ...
    for (let j = 0; j < tableContentMatrix[i].length; j++) {
        ...
        if (i == 0 || i == tableContentMatrix.length-1) { // изменение шапки таблицы
            td.style.fontWeight = 'bold';
        }

        if (j == 0) { // изменение шапки таблицы
            td.style.backgroundColor = '#f2f2f2';
        } else if (j == tableContentMatrix[i].length-1) { // изменение шапки таблицы
            td.style.backgroundColor = '#e3f3c4';
        } else {
    ...
    table.style.borderCollapse = 'collapse'; // чтобы границы ячеек не дублировались
